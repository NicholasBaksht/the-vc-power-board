...
/* Every family the generator writes belongs here, or its obsolete pages
   are written once and then served forever.

   company/ was missing and had accumulated 18 of them. Nine were names
   later held in data-company-aliases.js as two different companies each
   - Paladin, Casa, Pie, Liquid, Enigma, Ellis, Weave, Hubble, Ohana -
   so /company/paladin/ was still publishing a single merged profile for
   a name this site has explicitly refused to resolve. The other nine
   were pre-alias slugs that now duplicate a live page under the
   company's real name: planet vs planet-labs, temporal vs
   temporal-technologies, square-block vs block, recursion-pharmaceuticals
   vs recursion, bill-com, root-insurance, allbirds, pinduoduo, prenetics.

   capital-sources/ has no orphans today only because no angel has been
   removed yet. It is listed for the same reason. */
const MANAGED_PAGE_DIRS = ['firms', 'people', 'compare', 'companies', 'locations', 'stages',
                           'company', 'capital-sources'];
